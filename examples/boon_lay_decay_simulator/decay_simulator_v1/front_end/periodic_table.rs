...
impl DecaySimApp {
    ...
    pub fn periodic_table(&mut self, ui: &mut Ui) {
        ...
        // Period 5
        {
            ...
            let p = cell_pos(7, 5);  Self::ui_element_box_at_position(ui, Nuclide::Tc98, p.x, p.y); // longest-lived
            ...
            let p = cell_pos(13, 5); Self::ui_element_box_at_position(ui, Nuclide::In115,p.x, p.y); // very long-lived
        ...
        // Period 6 (main row)
        {
            ...
            let p = cell_pos(4, 6);  Self::ui_element_box_at_position(ui, Nuclide::Hf178, p.x, p.y); // stable
            ...
            let p = cell_pos(7, 6);  Self::ui_element_box_at_position(ui, Nuclide::Re185, p.x, p.y); // stable (Re-187 is very long-lived)
            ...
            let p = cell_pos(15,6);  Self::ui_element_box_at_position(ui, Nuclide::Bi209, p.x, p.y); // extremely long-lived
            let p = cell_pos(16,6);  Self::ui_element_box_at_position(ui, Nuclide::Po209, p.x, p.y); // longest-lived Po
            let p = cell_pos(17,6);  Self::ui_element_box_at_position(ui, Nuclide::At210, p.x, p.y); // longest-lived At
            let p = cell_pos(18,6);  Self::ui_element_box_at_position(ui, Nuclide::Rn222, p.x, p.y); // longest-lived Rn
        }

        // f-block: Lanthanides (Ce–Lu), rendered on an extra row below the main table
        {
            let f_start_group = 3usize;
            let lanth_period = 8usize; // visually below the main table
                                       // Ce–Lu (14 elements)
            ...
            let p = cell_pos(f_start_group + 3,  lanth_period); Self::ui_element_box_at_position(ui, Nuclide::Pm145, p.x, p.y); // longest-lived Pm
            ...
            let p = cell_pos(f_start_group + 13, lanth_period); Self::ui_element_box_at_position(ui, Nuclide::Lu175, p.x, p.y); // Lu-175 stable (Lu-176 very long-lived)
        }

        // Period 7 (main row)
        {
            let p = cell_pos(1, 7);  Self::ui_element_box_at_position(ui, Nuclide::Fr223, p.x, p.y);  // longest-lived Fr
        ...
        // actinides
        {
            ...
            let p = cell_pos(f_start_group + 4,  actin_period); Self::ui_element_box_at_position(ui, Nuclide::Pu244, p.x, p.y); // longest-lived Pu
            let p = cell_pos(f_start_group + 5,  actin_period); Self::ui_element_box_at_position(ui, Nuclide::Am243, p.x, p.y);
            let p = cell_pos(f_start_group + 6,  actin_period); Self::ui_element_box_at_position(ui, Nuclide::Cm247, p.x, p.y); // longest-lived Cm
            let p = cell_pos(f_start_group + 7,  actin_period); Self::ui_element_box_at_position(ui, Nuclide::Bk247, p.x, p.y); // longest-lived Bk
            let p = cell_pos(f_start_group + 8,  actin_period); Self::ui_element_box_at_position(ui, Nuclide::Cf251, p.x, p.y); // longest-lived Cf

            let p = cell_pos(f_start_group + 9,  actin_period);  Self::ui_element_box_at_position(ui, Nuclide::Es252, p.x, p.y); // Z=99
            let p = cell_pos(f_start_group + 10, actin_period); Self::ui_element_box_at_position(ui, Nuclide::Fm257, p.x, p.y); // longest-lived Fm
            let p = cell_pos(f_start_group + 11, actin_period); Self::ui_element_box_at_position(ui, Nuclide::Md258, p.x, p.y); // longest-lived Md
            let p = cell_pos(f_start_group + 12, actin_period); Self::ui_element_box_at_position(ui, Nuclide::No259, p.x, p.y); // longest-lived No
            let p = cell_pos(f_start_group + 13, actin_period); Self::ui_element_box_at_position(ui, Nuclide::Lr262, p.x, p.y); // longest-lived Lr
        }

        // Superheavy elements (Period 7, groups 4–18) — for reference, longest-lived nuclides
        {
            let p = cell_pos(4, 7);  Self::ui_element_box_at_position(ui, Nuclide::Rf267, p.x, p.y);
            let p = cell_pos(5, 7);  Self::ui_element_box_at_position(ui, Nuclide::Db268, p.x, p.y);
            let p = cell_pos(6, 7);  Self::ui_element_box_at_position(ui, Nuclide::Sg271, p.x, p.y);
...
impl Widget for ElementBox {
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        // Reserve space and set up interaction.
        ...
        let padding = 6.0;
        let small_text_size = (desired_size.y * 0.16).clamp(10.0, 16.0); // for Z and A
        let symbol_text_size = (desired_size.y * 0.48).clamp(18.0, 40.0); // main symbol

        // Positions:
        // - Proton number (Z): top-left
        // - Mass number (A): top-right (optional)
        // - Symbol: centered
        let top_left = Pos2::new(rect.left() + padding, rect.top() + padding);

        // Draw proton number Z.
        {
        ...
        // Draw element symbol centered.
        {
            ...
            // Center the symbol galley within the rect.
